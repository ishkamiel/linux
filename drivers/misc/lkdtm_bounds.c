 ...
 */
noinline static void mpxk_write_10_i(void *s0, void *s1, void *s2, void *s3, void *s4,
	void *s5, void *s6, void *s7, void *s8, void *s9, int i, int j, char c)
{
	if (i == 0) ((char *)s0)[j] = c;
	if (i == 1) ((char *)s1)[j] = c;
	if (i == 2) ((char *)s2)[j] = c;
...
void do_the_test(const int test_num) {
	...
	switch(test_num) {
		...
		case BND_FUNC10:
			...
			pr_info("attempting good ptr write\n");
			SET_IJ(8);
			mpxk_write_10_i(arr[0], arr[1], arr[2], arr[3], arr[4], arr[5],
				arr[6], arr[7], arr[8], arr[9], 2, 40 * 3 -1, 'C');
